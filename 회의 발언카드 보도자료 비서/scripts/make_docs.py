...
def build_report(d):
    ...
    if d.get("subtitle"):
        para(doc, d["subtitle"], size=10.5, align=CENTER, color=GRAY, after=6)
    if d.get("date_line"):
        para(doc, d["date_line"], size=10, align=RIGHT, color=GRAY, after=14)
...
def main():
    ...
    ap.add_argument("--outdir", default=None, help="출력 폴더 (미지정 시 입력 JSON과 같은 폴더 — 같은 건의 파일을 한 곳에 모음)")
    args = ap.parse_args()

    if args.press:
        press_path = args.press if os.path.isabs(args.press) else os.path.join(BASE, args.press)
        d = load(press_path)
        outdir = args.outdir or os.path.dirname(press_path)
        outdir = outdir if os.path.isabs(outdir) else os.path.join(BASE, outdir)
    ...
    if args.report:
        ...
        d = load(report_path)
        outdir = args.outdir or os.path.dirname(report_path)
        outdir = outdir if os.path.isabs(outdir) else os.path.join(BASE, outdir)
        os.makedirs(outdir, exist_ok=True)
